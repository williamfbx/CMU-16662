...
def CheckPointOverlap(pointsA, pointsB, axis):	
    ...
    proj_A = np.dot(pointsA, axis_normalized)
    proj_B = np.dot(pointsB, axis_normalized)
    ...
    overlap = (min_A <= max_B) and (min_B <= max_A)
...
def CheckBoxBoxCollision(pointsA, axesA, pointsB, axesB):
    ...
    for axisA in axesA:
        for axisB in axesB:
            ...
            if not np.allclose(cross_axis, 0, atol=1e-6):
